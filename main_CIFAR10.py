...
def main():
    ...
    optimizer = optim.SGD(parameters, lr=0.001, momentum=0.9, weight_decay=0.0005, nesterov=True) # Original
    ...
    trainset = torchvision.datasets.CIFAR10(root=data_root_path, train=True, download=True, transform=train_transform)
    trainloader = torch.utils.data.DataLoader(trainset, batch_size=args.batch_size, num_workers=args.workers, pin_memory=True)

    testset = torchvision.datasets.CIFAR10(root=data_root_path, train=False, download=True, transform=test_transform)
    ...
    print("Time taken to train the model ", end_time - start_time, file=stats_file)
...
class BarlowTwins(nn.Module):
    def __init__(self, args):
        ...
        match args.network:
            case 'ResNet18':
                self.backbone = ResNet18(10)
                sizes = [512] + list(map(int, args.projector.split('-')))
            case 'ResNet50':
                self.backbone = ResNet50(10)
                sizes = [2048] + list(map(int, args.projector.split('-')))
            case 'ResNet101':
                self.backbone = ResNet101(10)
                sizes = [2048] + list(map(int, args.projector.split('-')))
            case 'MobileNetV2':
                self.backbone = MobileNetV2(10)
                sizes = [1280] + list(map(int, args.projector.split('-')))
            case 'WideResNet28_10':
                self.backbone = WideResNet(depth=28, num_classes=10, widen_factor=10)
                sizes = [640] + list(map(int, args.projector.split('-')))
            case 'VGG19':
                self.backbone = VGG('VGG19', num_classes=10)
                sizes = [512] + list(map(int, args.projector.split('-')))
            case 'EfficientNetB2':
                self.backbone = EfficientNetB2(10)
