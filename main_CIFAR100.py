...
parser = argparse.ArgumentParser(description='LISAD CIFAR100 Training')
...
parser.add_argument('--batch-size', default=2048, type=int, metavar='N', help='mini-batch size') # 1024 - MobileNetv2 # 512 - ResNet50 WideResNet28-10 # 256 - ResNet101
...
parser.add_argument('--checkpoint-dir', default='exps_CIFAR100/', type=Path, metavar='DIR', help='path to checkpoint directory')
...
def add_random_gaussian_noise(x, var, device):
    gauss_x = x + (var**0.5)*torch.randn(x.shape[0], 3, 32, 32).cuda(device)
    return gauss_x
...
def main():
    ...
    optimizer = optim.SGD(parameters, lr=0.001, momentum=0.9, weight_decay=0.0005, nesterov=True)
    ...
    trainset = torchvision.datasets.CIFAR100(root=data_root_path, train=True, download=True, transform=train_transform)
    trainloader = torch.utils.data.DataLoader(trainset, batch_size=args.batch_size, num_workers=args.workers, pin_memory=True)

    testset = torchvision.datasets.CIFAR100(root=data_root_path, train=False, download=True, transform=test_transform)
...
class BarlowTwins(nn.Module):
    def __init__(self, args):
        ...
        match args.network:
            case 'ResNet18':
                self.backbone = ResNet18(100)
                sizes = [512] + list(map(int, args.projector.split('-')))
            case 'ResNet50':
                self.backbone = ResNet50(100)
                sizes = [2048] + list(map(int, args.projector.split('-')))
            case 'ResNet101':
                self.backbone = ResNet101(100)
                sizes = [2048] + list(map(int, args.projector.split('-')))
            case 'MobileNetV2':
                self.backbone = MobileNetV2(100)
                sizes = [1280] + list(map(int, args.projector.split('-')))
            case 'WideResNet28_10':
                self.backbone = WideResNet(depth=28, num_classes=100, widen_factor=10)
                sizes = [640] + list(map(int, args.projector.split('-')))
            case 'VGG19':
                self.backbone = VGG('VGG19', num_classes=100)
                sizes = [512] + list(map(int, args.projector.split('-')))
            case 'EfficientNetB2':
                self.backbone = EfficientNetB2(100)
                sizes = [320] + list(map(int, args.projector.split('-')))
